"""CLI/ACP 类 harness 接入桥 — 基于 ACP (Agent Client Protocol)。

通过 JSON-RPC over stdio 程序化拉起 ACP harness 新会话、发消息、收回复。
...
- API key 须注入环境变量（部分 harness 的 llm 提供方不认 credentials 里的扁平 key）

参考：ACP (Agent Client Protocol) 协议文档；ACP 运行目录由环境变量 AC_ACP_REPO 指定
协议要点（initialize → session/new → session/prompt → 收到回复）已在开发期于本地验证。

自测：python -m agent_community.platform.acp_harness_bridge
"""
...
REPO = os.environ.get("AC_ACP_REPO", "")  # ACP 运行目录（由环境变量注入，仓库不内置任何本地路径）
CMD = [p for p in os.environ.get("AC_ACP_COMMAND", "").split() if p]  # ACP 启动命令（由环境变量注入）
# API Key 一律从环境变量读取；仓库不内置任何本地凭据路径
PROTOCOL_VERSION = 1


def load_api_key() -> str:
    """从环境变量读取 API Key（不读取任何本地凭据文件）。"""
    return os.environ.get("DEEPSEEK_API_KEY", "") or os.environ.get("AC_AI_API_KEY", "")


@dataclass
class AcpSession:
    """一个 HA 会话（一个并行工作的"手"，即 ACP harness 的一个 session）。"""
    session_id: str
    cwd: str
...
class AcpHarnessBridge:
    """ACP 桥：spawn 一个 ACP harness server，管理多个会话。
    ...
    @classmethod
    def from_harness_info(cls, info) -> "AcpHarnessBridge":
        """从 HarnessInfo 构造桥：读 acp_command / acp_cwd，否则用环境变量中的默认命令。"""
        import shlex
    ...
    def start(self, api_key: Optional[str] = None) -> None:
        if not self.cmd:
            raise RuntimeError("未配置 ACP 启动命令：请通过 HarnessInfo.acp_command 或环境变量 AC_ACP_COMMAND 提供")
        env = dict(os.environ)
        env["DEEPSEEK_API_KEY"] = api_key or load_api_key()
        self.proc = subprocess.Popen(
            self.cmd, cwd=self.repo or None, env=env,
...
if __name__ == "__main__":
    ...
    print("[bridge] start ACP harness ...", flush=True)
    ...
    try:
        ...
        cwd = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "acp_ws"))
